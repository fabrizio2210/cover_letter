...
The `ai_querier` service consumes cover-letter jobs from Redis, uses Gemini for text generation and refinement, and persists the resulting cover-letter state into MongoDB.
...
3. Resolve the required MongoDB context.
4. Branch to cover-letter generation or cover-letter refinement.
5. Persist the resulting cover-letter state.
...
Job-scoring read paths and storage contracts are defined in `../ai_scorer/SPEC.md`.
...
3. Check `../ai_scorer/SPEC.md` for job-scoring contracts and ownership.
4. Check `../../go/internal/proto/common/common.proto` before changing persisted cover-letter fields.
5. Preserve exact field names in Redis payloads and MongoDB documents.
6. If you change shared contracts, update the Go API, Python worker, protobuf definitions, and both spec files in the same change set.
...
3. `../ai_scorer/SPEC.md` for job-scoring behavior handled by the separate scoring worker;
