...
```json
{
  ...
  "identity_id": "<identity hex object id>"  // optional for API-triggered scoring; required for crawler-triggered scoring
}
```
...
- `identity_id` is optional but strongly recommended for crawler-produced messages. When present, the worker resolves identity directly by `_id` in the per-user DB, bypassing company field inference.
- When `identity_id` is absent the worker falls back to inferring identity via `company.field` → `identities.field` linkage. If inference also fails, the job is recorded as `skipped` with reason `identity_not_found`.
...
- Messages without `user_id` or `job_id` are rejected and logged.
