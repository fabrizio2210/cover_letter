...
4. For each enabled identity preference, normalize configured metadata and determine score availability.
5. Normalize guarded preference fragments, classify evidence scope, expand the retrieval query, and rerank relevant description snippets.
6. Score each available preference using Ollama with reduced prompt context (title, normalized location, retrieved snippets).
7. Persist per-preference scores.
8. Compute deterministic aggregate ranking from stored scores and weights.
9. Update job scoring fields and lifecycle status.
...
| `SCORING_NUM_PREDICT` | none | No | Maximum tokens generated by each scoring request; unset preserves the model default |
| `SCORING_THINK` | none | No | Explicitly enable or disable model thinking for scoring requests; unset preserves the model default |
| `AUXILIARY_THINK` | none | No | Explicitly enable or disable thinking for JSON helper requests; unset preserves the model default |
| `QUERY_EXPANSION_MODEL` | `qwen2.5:1.5b` | No | Model that expands preference guidance for evidence retrieval |
| `NORMALIZE_JOB_LOCATION` | false | No | Normalize raw location metadata before scoring |
| `METADATA_NORMALIZATION_MODEL` | `qwen2.5:1.5b` | No | Model used for location normalization |
| `EXPLICIT_REMOTE_LOCATION` | false | No | Render normalized `remote` metadata as `fully remote` in scoring prompts |
| `NORMALIZE_PREFERENCE_GUIDANCE` | false | No | Enable guarded normalization of unresolved preference fragments |
| `PREFERENCE_NORMALIZATION_MODEL` | `qwen2.5:7b` | No | Model used to classify and normalize guarded preference fragments |
| `EVIDENCE_SCOPE_ROUTING` | none | No | Set to `llm` to classify metadata-only versus description evidence |
| `EVIDENCE_SCOPE_MODEL` | `qwen2.5:7b` | No | Model used for evidence-scope classification |
...
- When set, `SCORING_NUM_PREDICT` must be an integer greater than zero.
- When set, `SCORING_THINK` must be `true` or `false` (the aliases `1`/`0` and `yes`/`no` are also accepted).
- When set, `AUXILIARY_THINK` accepts the same boolean values and applies to all structured JSON helper requests.
