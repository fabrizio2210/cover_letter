...
class Workflow1IntegrationTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        db_name = os.getenv("DB_NAME", "cover_letter_workflow1_it")
    ...
    def setUp(self):
        ...
        try:
            self.database["companies"].insert_one(
                {
                    "_id": self.company_id,
                    "name": "Workflow One Corp",
                    "canonical_name": "workflow one corp",
    ...
    def _fake_fetch_jobs(self, provider, slug, config, session):
        return [
            common_pb2.Job(
                title="Workflow1 Engineer Role",
                description="Workflow1 job description.",
                location="Remote",
                platform=provider,
                external_job_id="wf1-test-1",
                source_url=f"https://example.com/{slug}/1",
            )
        ]

    def test_crawler_inserts_job_into_job_descriptions_collection(self):
        with patch(
            "src.python.web_crawler.crawler_ats_job_extraction.workflow.fetch_jobs",
            side_effect=self._fake_fetch_jobs,
        ):
        ...
        doc = self.database["job-descriptions"].find_one(
            {"platform": "greenhouse", "external_job_id": "wf1-test-1"}
        )
    ...
    def test_crawler_is_idempotent_on_recrawl(self):
        with patch(
            ...
            first = run_crawler_ats_job_extraction(
                self.database, self.config,
                identity_id=str(self.identity_id),
                identity_database=self.database,
            )
        ...
        with patch(
            ...
            second = run_crawler_ats_job_extraction(
                self.database, self.config,
                identity_id=str(self.identity_id),
                identity_database=self.database,
            )
