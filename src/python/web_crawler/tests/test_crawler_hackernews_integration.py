...
from src.python.web_crawler.crawler_hackernews.workflow import run_crawler_hackernews
...
class CrawlerCompanyDiscoveryMongoIntegrationTests(unittest.TestCase):
    ...
    def test_run_crawler_hackernews_persists_and_updates_companies(self):
        first_run_companies = [
            DiscoveredCompany(
                name="Acme, Inc.",
        ...
        with patch(
            "src.python.web_crawler.crawler_hackernews.workflow.get_enabled_adapters",
            return_value=[StubAdapter(companies=first_run_companies)],
        ):
            result = run_crawler_hackernews(self.database, self.config, str(self.identity_id))
        ...
        with patch(
            ...
            second_result = run_crawler_hackernews(self.database, self.config, str(self.identity_id))
