...
| Output queues | `CRAWLER_COMPANY_DISCOVERY_QUEUE_NAME`, `CRAWLER_ATS_JOB_EXTRACTION_QUEUE_NAME`, `CRAWLER_LEVELSFYI_QUEUE_NAME` |
...
One `WorkflowDispatchMessage` per target workflow pushed to each queue:
...
- Do **not** add MongoDB calls to this module.
...
- The `trigger_kind` field must remain `"public_crawl"` for dispatcher-originated messages.
