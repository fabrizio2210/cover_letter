...
from src.python.web_crawler.crawler_hackernews.workflow import (
    _WORKFLOW_ID,
    run_crawler_hackernews,
)
...
def worker_main(config: CrawlerConfig) -> None:
    ...
    while True:
        try:
            ...
            queue_item = cast(
                tuple[str, str] | None,
                redis_client.blpop([config.crawler_hackernews_queue_name], timeout=30),
            )
            if not queue_item:
                continue
            ...
            try:
                ...
                result = run_crawler_hackernews(database, config, identity_id, identity_database=user_database)
                ...
                publish_progress(
                    ...
                    status="completed",
                    workflow=_WORKFLOW_ID,
                    estimated_total=max(result.discovered_count, 1),
                    completed=result.inserted_count + result.updated_count,
                    started_at=started_at,
                    finished_at=finished_at,
                    message="Hacker News company discovery completed",
                    workflow_id=_WORKFLOW_ID,
                    workflow_run_id=workflow_run_id,
                )
            except Exception as exc:
                logger.exception("crawler_hackernews failed for identity %s: %s", identity_id, exc)
                finished_at = utc_timestamp()
                publish_progress(
                    redis_client,
                    config,
                    ...
                    started_at=started_at,
                    finished_at=finished_at,
                    message="Hacker News company discovery failed",
                    reason="run_failed",
                    workflow_id=_WORKFLOW_ID,
                    workflow_run_id=workflow_run_id,
                )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the crawler_hackernews workflow worker")
