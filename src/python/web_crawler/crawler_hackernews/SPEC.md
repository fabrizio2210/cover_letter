# crawler_hackernews — Specification

**Authoritative reference for the `crawler_hackernews` package.**
...
The `crawler_hackernews` package discovers employer companies from Hacker News "Who is Hiring" threads, deduplicates and upserts them into MongoDB, then emits `CompanyDiscoveryEvent` messages for every company that still needs ATS enrichment.
...
| Item | Value |
|---|---|
| Workflow module | `src.python.web_crawler.crawler_hackernews.workflow` |
| Worker module | `src.python.web_crawler.crawler_hackernews.worker` |
| Docker CMD | `python -m src.python.web_crawler.crawler_hackernews.worker --worker` |
| Input queue | `CRAWLER_HACKERNEWS_QUEUE_NAME` (default `crawler_hackernews_queue`) |
...
| Workflow ID | `crawler_hackernews` |
