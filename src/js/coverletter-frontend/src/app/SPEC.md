...
- for each scored job card, show every per-preference entry from `preference_scores` as the pair `preference_key` + `score` (`0..5` when `score_available=true`, otherwise N/A);
